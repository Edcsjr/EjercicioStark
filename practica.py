...
def generar_codigo_heroe(id_heroe:int,genero_heroe:str):
    ...
    if genero_heroe.upper() not in ["M","NB","F"]:
        ...
        if genero_heroe.upper()=="NB":
            ...
            print(codigo)
            return codigo
#generar_codigo_heroe(12345678,"NB")
...
def agregar_codigo_heroe(heroe:dict,id_heroe:int)-> bool:
    ...
    if generar_codigo_heroe(id_heroe,heroe["genero"])=="N/A":
        print("supera 10 caracteres")
        return False
    else:
        heroe["codigo_heroe"]=generar_codigo_heroe(id_heroe,heroe["genero"]) #Error debido a la funcion que necesita | funcion anterior
...
def stark_generar_codigos_heroes(lista:list):
    ...
    if type(lista) != list or len(lista) == 0:
        print("Error no recibe ninguna lista")
    
    for heroe in lista:
        
        if type(heroe) != dict:
            print("El origen de datos no contiene el formato correcto")
        elif not "genero" in heroe:
            print("El origen de datos no contiene el formato correcto genero no esta en dicc")
        else:
            cont_heroes+=1
            id_heroe=lista.index(heroe)+1
            agregar_codigo_heroe(heroe,id_heroe)
            
    primer_cod=lista[0]["codigo_heroe"]
    ultimo_cod=lista[-1]["codigo_heroe"]
    print(f"{cont_heroes} representa la cantidad de codigos generados \n Se asignaron {cont_heroes} codigos")
    print(f"El codigo del primer heroe es {primer_cod} ultimo codigo {ultimo_cod} ")  # consultar si el codigo generado es el correcto el ultimo es 24 no 1224
...
encontrado:
● Si contiene carácteres no numéricos retornar -1 listo
● Si el número es negativo se deberá retornar un -2 listo
...
def sanitizar_flotante(numero_str:str):
    ...
    if  not numero.startswith("-")  and numero.replace(".","").isalpha():
        return -1
    
    # Verificar si el número es negativo
    if numero_str.startswith('-') and numero[1:].replace(".","").isnumeric():
        return -2
...
def sanitizar_string(valor_str:str,valor_por_defecto:str="-"):
    ...
    valor_p_d=valor_por_defecto.strip()
    
    if not valor_str and valor_por_defecto != '-':
        return valor_p_d.lower()
    
    valor=valor_s.replace("/"," ")

    if valor_s.replace("/","").isalpha():
        return valor.lower()
    else:
        return "N/A"

#print(sanitizar_string("  h/o/l/@"))
...
def sanitizar_dato(heroe:dict,clave:str,tipo_de_dato:str):
    
    clave_str=clave.lower()
    tipo_d_d=tipo_de_dato.lower()
    
    if  clave_str not in heroe:
        print("La clave especificada no existe en el heroe")
        return False
    if tipo_d_d not in ["flotante","entero","string"]:
        print("Tipo de dato no reconocido")
        return False

    
    match (tipo_d_d):   # es necesario validar el tipo de la clave con el typo del dato
        case "flotante":
            sanitizar_flotante(heroe[clave_str])
            #print(sanitizar_flotante(heroe[clave_str]))
            
            return True

        case "string":
            sanitizar_string(heroe[clave_str])
            #print(sanitizar_string(heroe[clave_str]))
            
            return True
        
        case "entero":
            sanitizar_entero(heroe[clave_str])
            #print(sanitizar_entero(heroe[clave_str]))
...
#print(sanitizar_dato(lista_personajes[0],"nombre","flotante"))
...
"""4.1
Crear la función ‘generar_indice_nombres’ la cual recibirá como
parámetro:
● lista_heroes: la lista de personajes
...
def generar_indice_nombres(lista:list):
    ...
    if type(lista) !=list:
        print("El origen de datos contiene un formato incorrecto")
    ...
    for heroe in lista:
        if type(heroe) != dict:
            print("Error se encontro un elemento de tipo no diccionario" )
        elif "nombre" not in heroe:
            print("Hay elementos sin clave nombre")
            break
        else:
            lista2 +=heroe["nombre"].split()
    return lista2
...
def stark_imprimir_indice_nombre(lista:list):
    ...
    for elemento in generar_indice_nombres(lista):
        imprimir_lista +=f"{elemento}-"
...
def convertir_cm_a_mtrs(valor_cm:int):
    ...
    if type(valor_cm)== float and valor_cm > 0:
        conversion=valor_cm/100
        
        return conversion
    
    elif type(valor_cm)== float and valor_cm < 0:
        return -1
